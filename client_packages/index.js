...
function requestComponentDrawableVariations(male, prop = false) {
    const gender = male !== null && male !== void 0 ? male : mp.players.local.isMale();
    mp.players.local.model = mp.game.joaat(`mp_${gender ? 'm' : 'f'}_freemode_01`);
    const drawableVariations = [];
    for (let x = 1; x < 12; x++) {
        if(!prop) {
            drawableVariations.push(mp.players.local.getNumberOfDrawableVariations(x));
        } else {
            drawableVariations.push(mp.players.local.getNumberOfPropDrawableVariations(x));
        }
    }
